 ...
 * {@code ~/.runelite/hit-distribution/}. All access is synchronized: the client thread writes,
 * the Swing thread reads snapshots.
 */
@Slf4j
@Singleton
public class HitDistributionStore
{
	static final File HISTORY_DIR = new File(RuneLite.RUNELITE_DIR, "hit-distribution");
	...
	@Inject
	public HitDistributionStore(Gson gson)
	{
		this.gson = gson;
	}
	...
	public synchronized void load(String player)
	{
		...
		dirty = false;
		revision++;
		log.debug("Loaded hit distribution for {} from {}: {} contexts, {} fights",
	...
	public synchronized void save()
	{
		...
		{
			log.warn("Unable to save hit distribution to {}", file, e);
		}
	}
	...
	private HistoryData read(File source)
	{
		...
		catch (IOException | RuntimeException e)
		{
			...
			log.warn("Unable to read hit distribution {}; starting a fresh one", source, e);
